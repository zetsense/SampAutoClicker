...
local imguiHandler = MoonCore.class("imguiHandler", {
    ...
    public = {
        ...
        renderClickerPanel = function(self, config)
            ...
            --   
            imgui.SetCursorPosX(20)
            imgui.TextColored(self.colors.text, u8"Êîëè÷åñòâî êëèêîâ:")
            imgui.SetCursorPosX(20)
            ...
            -- Ïîäñêàçêà ïðè íàâåäåíèè íà ñëàéäåð êîëè÷åñòâà êëèêîâ
            if imgui.IsItemHovered() then
                imgui.BeginTooltip()
                imgui.PushTextWrapPos(300)
                imgui.TextColored(imgui.ImVec4(1, 1, 1, 1), u8"Êîëè÷åñòâî êëèêîâ, âûïîëíÿåìûõ çà îäèí öèêë")
                imgui.TextColored(imgui.ImVec4(0.8, 0.8, 0.8, 1.0), u8"Ïðè óâåëè÷åíèè ïàðàìåòðà ïðîãðàììà áóäåò âûïîëíÿòü íåñêîëüêî êëèêîâ ïîäðÿä")
                imgui.TextColored(imgui.ImVec4(0.7, 0.9, 0.7, 1.0), string.format(u8"Òåêóùåå çíà÷åíèå: %d êëèêîâ", settings.clickCount))
                imgui.PopTextWrapPos()
                imgui.EndTooltip()
            end
